...
export default class ToyCarLoader {
    ...
    _processBlocks(blocks, precisePhysicsModels) {
        blocks.forEach(block => {
            ...
            // Si no hay un GLB para este recurso, solo abortamos para bloques normales.
            // Para bloques tipo 'coin' creamos un modelo fallback sencillo (cilindro dorado).
            if (!glb && !block.name.startsWith('coin')) {
                console.warn(`Modelo no encontrado: ${resourceKey}`);
                return;
            }

            const model = glb ? glb.scene.clone() : null;

            // Validar que el modelo existe antes de acceder a userData
            if (model) {
            ...
            //  Si es un premio (coin) - procesar ANTES que otras lógicas
            if (block.name.startsWith('coin')) {
                // Si no había modelo GLB, crear un modelo fallback (cilindro dorado)
                let prizeModel = model;
                if (!prizeModel) {
                    const coinGeom = new THREE.CylinderGeometry(0.35, 0.35, 0.12, 16);
                    ...
                    group.add(coinMesh);
                    prizeModel = group;
                    prizeModel.userData = { levelObject: true };
                }
                ...
                if (prize.model && prize.model.userData) {
            ...
            // Si llegamos aquí, no es una moneda y necesita modelo GLB válido
            if (!model) {
                console.warn(`❌ Modelo GLB no encontrado para: ${resourceKey}`);
                return;
            }
